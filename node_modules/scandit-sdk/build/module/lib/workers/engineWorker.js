...
export function engine() {
    ...
    function loadLibrary(deviceId, libraryLocation, locationPath) {
        ...
        Module = {
            ...
            instantiateWasm: (importObject, successCallback) => {
                ...
                // istanbul ignore if
                if (wasmJSVersion !== "4.4.1") {
                    console.error(`The Scandit SDK Engine library JS file found at ${jsURI} seems invalid: ` +
                        `expected version doesn't match (received: ${wasmJSVersion}, expected: ${"4.4.1"}). ` +
    ...
    function getLibraryLocationURIs(libraryLocation) {
        ...
        if (/^https?:\/\/([^\/.]*\.)*cdn.jsdelivr.net\//.test(libraryLocation)) {
            libraryLocation = "https://cdn.jsdelivr.net/npm/scandit-sdk@4.4.1/build/";
            cdnURI = true;
        }
        else if (/^https?:\/\/([^\/.]*\.)*unpkg.com\//.test(libraryLocation)) {
            libraryLocation = "https://unpkg.com/scandit-sdk@4.4.1/build/";
            cdnURI = true;
        }
        if (cdnURI) {
            return {
        ...
        return {
            jsURI: `${libraryLocation}scandit-engine-sdk.min.js?v=4.4.1`,
            wasmURI: `${libraryLocation}scandit-engine-sdk.wasm?v=4.4.1`
        };
    }
