...
test.serial("adjustCamerasFromMainCameraStream", async (t) => {
    ...
    let cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream({}, cameras), undefined);
    t.is(cameras[0].cameraType, Camera.Type.BACK);
    t.is(cameras[1].cameraType, Camera.Type.FRONT);
    cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("1", "environment", ""), cameras), cameras[0]);
    t.is(cameras[0].cameraType, Camera.Type.BACK);
    t.is(cameras[1].cameraType, Camera.Type.FRONT);
    cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("2", "environment", ""), cameras), cameras[1]);
    t.is(cameras[0].cameraType, Camera.Type.BACK);
    t.is(cameras[1].cameraType, Camera.Type.BACK);
    cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("2", "user", ""), cameras), undefined);
    t.is(cameras[0].cameraType, Camera.Type.BACK);
    t.is(cameras[1].cameraType, Camera.Type.FRONT);
});
test.serial("adjustCamerasFromMainCameraStream (unknown cameras)", async (t) => {
    ...
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("4", "environment", ""), cameras), cameras[0]);
    t.is(cameras[0].cameraType, Camera.Type.BACK);
    t.is(cameras[1].cameraType, Camera.Type.FRONT);
    cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("5", "environment", ""), cameras), cameras[1]);
    t.is(cameras[0].cameraType, Camera.Type.FRONT);
    t.is(cameras[1].cameraType, Camera.Type.BACK);
    cameras = originalCameras.map(camera => {
        return { ...camera };
    });
    t.is(CameraAccess.adjustCamerasFromMainCameraStream(getFakeMediaStreamTrack("5", "user", ""), cameras), undefined);
    t.is(cameras[0].cameraType, Camera.Type.FRONT);
    t.is(cameras[1].cameraType, Camera.Type.BACK);
});
