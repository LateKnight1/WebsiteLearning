...
function checkBrowserCompatibility(): any {
  return {
    fullSupport: true,
    scannerSupport: true,
    missingFeatures: []
...
test.serial("addListener", async t => {
  ...
  resetStubs();
  const s: Scanner = new Scanner();
  const onSpy: sinon.SinonSpy<["ready", () => void], Scanner> = sinon.spy(s, "on");
...
test.serial("processImage calls", async t => {
  ...
  const s: Scanner = new Scanner();
  resetStubs();
  let error: Error = await t.throwsAsync(s.processImage(new Uint8ClampedArray(0)));
...
test.serial("processImage highQualitySingleFrameMode calls", async t => {
  ...
  const s: Scanner = new Scanner();
  resetStubs();
  let error: Error = await t.throwsAsync(s.processImage(new Uint8ClampedArray(0), true));
  t.is(error.name, "NoImageSettings");
  t.is((<any>s).workerScanRequestId, 0);
  t.is((<any>s).workerScanQueueLength, 0);
  t.false(s.isBusyProcessing());
  ...
  t.false(postMessageStub.called);
  resetStubs();
  const imageData: Uint8ClampedArray = Uint8ClampedArray.from({ length: 24 }, () => {
    return Math.floor(Math.random() * 255);
  });
  // tslint:disable-next-line: no-floating-promises
...
test.serial("processImage calls with ImageData", async t => {
  async function getImageFromCanvas(canvasElement: HTMLCanvasElement): Promise<HTMLImageElement> {
  ...
  canvas.width = 3;
  canvas.height = 2;
  let imageData: HTMLImageElement = await getImageFromCanvas(canvas);
  s.applyImageSettings({
    width: 3,
    height: 2,
    format: ImageSettings.Format.RGBA_8U
...
test.serial("processImage scan results", async t => {
  ...
  const imageData1: Uint8ClampedArray = Uint8ClampedArray.from({ length: 24 }, () => {
    return Math.floor(Math.random() * 255);
  });
  const processImage1: Promise<ScanResult> = s.processImage(imageData1); // 3 * 2 * 4
  t.true(s.isBusyProcessing());
  const imageData2: Uint8ClampedArray = Uint8ClampedArray.from({ length: 24 }, () => {
    return Math.floor(Math.random() * 255);
  });
  const processImage2: Promise<ScanResult> = s.processImage(imageData2); // 3 * 2 * 4
  const imageData3: Uint8ClampedArray = Uint8ClampedArray.from({ length: 24 }, () => {
    return Math.floor(Math.random() * 255);
  });
  const processImage3: Promise<ScanResult> = s.processImage(imageData3); // 3 * 2 * 4
  ...
  const error: Error = await t.throwsAsync(processImage2);
...
test.serial("createParserForFormat & parseString", async t => {
  ...
  const error: Error = await t.throwsAsync(parseString2);
  t.deepEqual(error.message, "example_error (123)");
