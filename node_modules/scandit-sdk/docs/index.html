	...
	<div class="row">
		<div class="col-8 col-content">
			<div class="tsd-panel tsd-typography">
				...
				<p>You can use the <a href="https://jsdelivr.com">jsDelivr</a> or <a href="https://unpkg.com">UNPKG</a> CDN to easily <a href="https://semver.npmjs.com">specify a certain version (range)</a> and include our library as follows (example for <code>4.4.2</code>: the current version automatically retrieved when specifying a <code>4.x</code> version; check our latest available library <a href="https://docs.scandit.com/stable/web/CHANGELOG.md">here</a>):</p>
				<pre><code class="language-html"><span class="hljs-tag">&lt;<span class="hljs-name">script</span> <span class="hljs-attr">src</span>=<span class="hljs-string">"https://cdn.jsdelivr.net/npm/scandit-sdk@4.x"</span>&gt;</span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
<span class="hljs-comment">&lt;!-- or --&gt;</span>
<span class="hljs-tag">&lt;<span class="hljs-name">script</span> <span class="hljs-attr">src</span>=<span class="hljs-string">"https://unpkg.com/scandit-sdk@4.x"</span>&gt;</span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span></code></pre>
    ...
    <span class="hljs-keyword">return</span> <span class="hljs-built_in">string</span> + <span class="hljs-string">`<span class="hljs-subst">${Barcode.Symbology.toHumanizedName(barcode.symbology)}</span>: <span class="hljs-subst">${barcode.data}</span>\n`</span>;
				...
				<ul>
					<li><p>How big is the library?</p>
						<p>The main library is currently about 90kB (minified, gzipped), while the external Scandit Engine library is around 42kB + 925kB (JS + WASM, gzipped). Keep in mind that you will not need to wait for these files to download to display your webpage / web application, they can be downloaded and prepared in the background without affecting the rest of the application (the Scandit Engine library is also always automatically downloaded and prepared asynchronously in a separate thread). On browsers supporting caching of WebAssembly compiled data, given resources served with correct standard HTTP caching headers and statuses, the Scandit Engine library WASM file can be pre-compiled and cached to be reused for subsequent loads.</p>
					</li>
					<li><p>Why is only Safari fully supported on iOS?</p>
						<p>This is (for now) a limitation imposed by Apple, which doesn&#39;t yet allow any external browser using WKWebView from accessing device cameras for video streaming, see <a href="https://forums.developer.apple.com/thread/88052">this discussion</a>. As an alternative/workaround, &quot;single image mode&quot; can be used to scan pictures in other browsers on this platform.</p>
					</li>
					<li><p>How well are barcodes recognized?</p>
						<p>This obviously depends from the specifics of the camera being used, but in general the performance of this library is only slightly inferior to what is provided by Scandit&#39;s native SDKs. Currently it&#39;s not possible on all major browsers to (efficiently) use WebGL instructions inside WebWorkers, meaning that advanced GPU control and functions provided to our library can be limited. The situation will improve over time as more of the features present in the native SDK are implemented/enabled and as browsers allow more control over the GPU and camera.</p>
					</li>
					...
					<li><p>Why can I sometimes see &quot;Intervention&quot; verbose messages about blocked vibrate calls in the browser&#39;s debugger console?</p>
					...
					<li><p>Why can I sometimes see &quot;Intervention&quot; verbose messages about non-passive event listeners in the browser&#39;s debugger console?</p>
						<p>That is normal behaviour and won&#39;t affect final users in any way: we need to actively listen for scroll events inside the <code>BarcodePicker</code> component in order to provide our camera zoom functionality on supported browsers/devices. Rest assured that due to their limited scope and functionality these listeners won&#39;t actually affect the performance of your webpage in any way.</p>
					</li>
					<li><p>Why can I sometimes see warning messages saying &quot;The AudioContext was not allowed to start&quot; in the browser&#39;s debugger console?</p>
						<p>That is normal behaviour and won&#39;t affect final users in any way: some browsers won&#39;t allow audio to be played by our library (on scan) if the user hasn&#39;t first touched/clicked the page, and will block the first audio initialization calls (outside from normal JavaScript code execution). Audio will however play as expected after the first user interaction.</p>
					</li>
					...
					<li><p>Why do I randomly get &quot;Out of bounds memory access&quot; or &quot;None: abort(6)&quot; errors?</p>
						<p>This is caused by a iOS 11.2.2 / 11.2.5 / 11.2.6 <em>Safari</em> browser bug, due to <em>WebKit</em> memory access problems introduced by this specific iOS versions by Apple, as indicated in the <a href="#important-notes">Important Notes</a> section. This is unfortunately out of our control, but a fix for it has already been released by Apple in iOS version 11.3.</p>
					</li>
					<li><p>Why do I keep getting asked for camera access permissions?</p>
						<p>Each browser has slightly different privacy rules for the situations in which to ask the user for access to their camera feed. Nonetheless all browsers usually let users store (&quot;remember&quot;) their permissions for each website or web application, so that these permissions don&#39;t need to be asked again. In order to ensure permissions are stored correctly in any situation, your website / web application should be served via HTTPS.</p>
					</li>
					<li><p>How does &quot;single image mode&quot; work?</p>
						<p>This mode is meant as an alternative/fallback mode for a <code>BarcodePicker</code> to provide single camera pictures to be scanned. It still performs all operations locally in the browser, but trades off continuous camera stream access for (more high quality) single snapshot scanning; this results in less browser features needed for the library to work and extended browser support. In &quot;single image mode&quot; a specially set UI is provided which enables users to click/tap to directly take a picture with the camera (mobile/tablet) or upload a file (desktop), this picture is then scanned and the results are provided. In this mode special camera access permissions don&#39;t have to be requested.</p>
					</li>
					<li><p>Why does Safari use more and more memory until it crashes, causing the page to reload with the error &quot;A problem occurred with this webpage so it was reloaded&quot;?</p>
						<p>There could be two issues - both can be remedied. Firstly, a general bugfix regarding memory usage in <em>Safari</em> has been released with library version 3.1.2. Secondly, there&#39;s currently a problem in <em>Safari</em>&#39;s own debugger, which causes it to use an increasing amount of memory while recording metrics. The issue is specifically caused by recording data through the debugger&#39;s &quot;Javascript &amp; Events&quot; event category, under &quot;Timelines&quot;.  When using the debugger, please disable &quot;Javascript &amp; Events&quot; recording, or pause the scanning process while recording information.</p>
					</li>
					<li><p>Why can&#39;t I access the camera in full HD resolution in Safari?</p>
						<p>Older <em>Safari</em> versions provide limited support to access audio and video media devices via standard web technologies (<em>WebRTC</em>): the available maximum resolution was capped at 1280x720. This is unfortunately out of our control, but a fix for it has already been released by Apple in iOS version 12.3.</p>
					</li>
					<li><p>Why do I get a &quot;DOMException&quot; error and can&#39;t access the camera in an iframe?</p>
						<p>If you&#39;re using the library inside a cross-origin iframe element you have to make sure you set the needed <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Feature_Policy#Specifying_your_policy">feature policies</a>, required by some browsers. In our case case the needed policy needs to be added to the iframe as: <code>allow=&quot;camera&quot;</code>.</p>
					</li>
					<li><p>Why do I get a &quot;DOMException&quot; error and can&#39;t access the camera when accessing my website at http://...?</p>
						<p>You must access your website / web application served from a <a href="https://www.chromium.org/Home/chromium-security/prefer-secure-origins-for-powerful-new-features">secure origin</a> (HTTPS or local IP) in order to access video camera video streaming due to browser restrictions. For development purposes you can set up a self-signed certificate, proxy, or just quickly set up a service like <a href="https://ngrok.com/">ngrok</a> to instantly get a secure URL to your localhost server.</p>
					</li>
				</ul>
				...
				<pre><code class="language-bash">yarn watch</code></pre>
				...
				<pre><code class="language-bash">yarn unit</code></pre>
